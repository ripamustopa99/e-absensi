...
type ApiRiwayatResponse = {
  success: true;
  data: { tahun: number; bulan: number; sessions: SessionRiwayat[] };
};
...
function deriveCardStatus(
  ...
  const isEditable = nowMinutes >= startMinutes - 15 && nowMinutes <= endMinutes + 15;
...
export default function AbsensiPage() {
  ...
  const [holidayToday, setHolidayToday] = useState<{ jenis: string; keterangan: string } | null>(null);
  ...
  const [tahunAjaranList, setTahunAjaranList] = useState<{ id: string; label: string; isAktif: boolean }[]>([]);
  const [searchRiwayat, setSearchRiwayat] = useState("");
  
  const [selectedJadwal, setSelectedJadwal] = useState<AttendanceCard | null>(null);
  ...
  useEffect(() => {
    const fetchTahunAjaran = async () => {
      try {
        const res = await api.get<{ success: true; data: { id: string; label: string; isAktif: boolean }[] }>("/tahun-ajaran");
        if (res.data.success && res.data.data.length > 0) {
          setTahunAjaranList(res.data.data);
          const active = res.data.data.find((t) => t.isAktif) || res.data.data[0];
          if (active) {
            const firstYear = parseInt(active.label.split("/")[0] ?? String(now.getFullYear()), 10);
    ...
    const fetchSchedulesAndHoliday = async () => {
      try {
        const [res, holidayRes] = await Promise.all([
          ...
          api.get<{ success: boolean; data: { isHoliday: boolean; holiday: any } }>(
        ...
        const todaySchedules = res.data.data
          ...
          .sort((a, b) => timeToMinutes(a.jamMulai) - timeToMinutes(b.jamMulai));
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-start gap-6 shadow-sm">
          <div className="flex items-center gap-6 text-[13px] font-medium text-[var(--text-secondary)]">
            ...
            <span>
              Belum Absen:{" "}
              <strong className="text-amber-500">{summaryHariIni.belum}</strong>
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-[var(--text-secondary)]">Menampilkan 7 sesi absensi terbaru</p>
            </div>
          </div>
          <Link
            href="/guru/riwayat"
          ...
          <div className="space-y-5">
            <div className="bg-[var(--surface-subtle)] border border-[var(--border)] rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-3 text-[13px]">
                ...
                <span className="font-semibold text-[var(--text-primary)]">{todayDateStr}</span>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <Clock size={16} className="text-blue-500" />
                <span className="font-semibold text-[var(--text-primary)]">{selectedJadwal.jamMulai} - {selectedJadwal.jamSelesai} WIB</span>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <BookOpen size={16} className="text-amber-500" />
                <span className="font-semibold text-[var(--text-primary)]">Jenjang {selectedJadwal.tahunAjaran ? selectedJadwal.tahunAjaran.label : ''} - Tingkat {selectedJadwal.tingkatList.join(", ")}</span>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <CheckSquare size={16} className="text-purple-500" />
                <span className="font-semibold text-[var(--text-primary)]">{selectedJadwal.mapel.nama}</span>
              </div>
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer group pt-2">
              <div className="relative flex items-center justify-center mt-0.5">
                ...
                />
                <div className={`w-5 h-5 rounded border-2 transition-all flex items-center justify-center ${isReadyToTeach ? "bg-primary border-primary" : "border-[var(--border)] group-hover:border-primary"}`}>
                  {isReadyToTeach && <CheckSquare size={14} className="text-white" />}
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {isSubmittingAbsenGuru ? (
                  <>
                    <Loader2 size={16} className="animate-spin" /> Mengarahkan...
                  </>
                ) : (
                  <>
...
function ScheduleCard({
  ...
  > = {
    ...
    BELUM_WAKTUNYA: {
      ...
      className: "bg-[var(--surface-subtle)] border-[var(--border)] text-[var(--text-tertiary)]",
    },
  };
